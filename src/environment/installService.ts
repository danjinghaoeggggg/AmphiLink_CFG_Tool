      ...
      return tool === 'gdb'
        ...
        : {
            ...
            command: [
              ...
              '$release = Invoke-RestMethod -Uri "https://api.github.com/repos/openocd-org/openocd/releases/tags/latest"',
              '$asset = $release.assets | Where-Object { $_.name -match "^openocd-[0-9a-f]+-i686-w64-mingw32\\.tar\\.gz$" } | Select-Object -First 1',
              'if (-not $asset) { throw "OpenOCD Windows prerelease asset was not found." }',
              '$url = $asset.browser_download_url',
              '$archive = Join-Path $env:TEMP "openocd-latest.tar.gz"',
              '$install = "C:\\OpenOCD"',
              'Invoke-WebRequest -Uri $url -OutFile $archive',
              '$expectedHash = if ($asset.digest -match "^sha256:(.+)$") { $Matches[1] } else { $null }',
              'if ($expectedHash -and (Get-FileHash -Algorithm SHA256 $archive).Hash -ne $expectedHash) { throw "OpenOCD archive checksum verification failed." }',
